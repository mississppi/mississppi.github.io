...
```
//配列
var arrayData: [String] = ["apple", "banana"]
print(arrayData)

//追加、削除
arrayData.append("orange")
let removeData = arrayData.remove(at: 2)

//取り出し
let firstData = arrayData[0]
...
```
//辞書型
var scores: [String: Int] = ["alice": 90, "bob": 75, "Charlie": 89]
print(scores)
...
DIc から値を取得するときは、必ず optional になる。(Int?)。Optional Binding という基礎の考え方

### 💡 Optional Binding とは

「nil じゃなかったら、この変数を使って処理を続ける」の仕組み

```
var opName: String? = "taro"
var opAge: Int? = 30
...
greetUser(name: "hanako", message: "hello")
greetUser(name: nil, message: "hello")
```

---

SwiftUI と CoreData を使って開発していく中でのメモ。思い出しやすいように、よく使うパターン・お作法をまとめ

# Swift プロパティラッパーまとめ

| ラッパー           | 主な用途                                               | 再生性の挙動                                                                                     |
| ------------------ | ------------------------------------------------------ | ------------------------------------------------------------------------------------------------ |
| `@ObservedObject`  | 親など外部から渡された `ObservableObject` を監視       | ビュー再生成で親が新しいインスタンスを渡すと差し替わる（再初期化され得る）                       |
| `@StateObject`     | ビューが ViewModel を生成・所有し、ライフタイムを保証  | 初回のみ初期化され、ビューの再生成でも同じインスタンスを保持（再初期化しない）                   |
| `@State`           | ビュー内部のローカルな小さな値（String/Bool 等）を保持 | ビューのライフサイクル中に保持。ビュー破棄 → 再生成で初期値に戻る可能性あり                      |
| `ObservableObject` | クラスで定義するデータソース（`@Published` を使う）    | ラッパーではない。誰が所有するか（`@ObservedObject`/`@StateObject`）によって再生成の影響を受ける |

親から viewModel を渡しているなら問題ありません。ビュー側で生成しているなら @StateObject に切り替えてください。
...
| 項目                 | 説明                                                                                      | コード例                                         |
| -------------------- | ----------------------------------------------------------------------------------------- | ------------------------------------------------ |
| **`onAppear`**       | View が画面に表示された**直後**に一度だけ処理を実行する。                                 | `  .onAppear { print("Hello") }`                 |
| **`init()`**         | View のインスタンスが**生成されるタイミング**で最初に 1 回だけ呼ばれる特別な関数。        | `  init() { _viewModel = StateObject(...) }`     |
| **`dismiss`**        | `.sheet` や `.popover` などで表示された画面を**閉じる**ために、環境変数から取得して使う。 | `  Button { dismiss() }`                         |
| **`NavigationLink`** | 次の画面へ遷移するための UI 要素。                                                        | `  NavigationLink("次へ") { DestinationView() }` |
|                      |                                                                                           |
|                      |                                                                                           |
|                      |                                                                                           |
|                      |                                                                                           |
...
```
/Users/iro/Library/Containers/xxxxxxxxxxx.Argonautes/Data/Library/Application Support/Argonautes
```
